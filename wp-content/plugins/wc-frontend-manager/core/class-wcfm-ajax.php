...
class WCFM_Ajax {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_wcfm_bulk_order_status', array(&$this, 'wcfm_bulk_order_status'));
    ...
    public function wcfm_bulk_order_status() {
        global $WCFM;
        ...
        $order_ids    = isset($_POST['order_ids']) ? array_filter(array_map('absint', (array) $_POST['order_ids'])) : [];
        $order_status = isset($_POST['order_status']) ? wc_clean($_POST['order_status']) : '';
        $new_status   = str_replace('wc-', '', $order_status);

        if (empty($order_ids)) {
            wp_send_json_error(__('No orders selected.', 'wc-frontend-manager'));
            wp_die();
        }
        if (!wc_is_order_status($order_status)) {
            wp_send_json_error(__('Invalid order status.', 'wc-frontend-manager'));
        ...
        wp_send_json_success(['message' => sprintf(
            _n('%d order updated to %s.', '%d orders updated to %s.', $updated, 'wc-frontend-manager'),
            $updated,
            wc_get_order_status_name($new_status)
        )]);
        wp_die();
