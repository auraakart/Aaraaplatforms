	...
	public function processing() {
		...
		if (!empty($wcfm_orders_array)) {
			...
			foreach ($wcfm_orders_array as $wcfm_orders_single) {
				...
				// Status
				$datatable_json['data'][$index][] = apply_filters('wcfm_order_status_display', '<span class="order-status tips wcicon-status-default wcicon-status-' . sanitize_title($order_status) . ' text_tip" data-tip="' . wc_get_order_status_name($order_status) . '" data-order-id="' . esc_attr($the_order->get_id()) . '"></span>', $the_order);
