...
<div class="collapse wcfm-collapse" id="wcfm_orders_listing">
  ...
  <div class="wcfm-page-headig">
	...
	<div class="wcfm-collapse-content">
	  ...
	  <style>
	  #wcfm-orders td:first-child input[type="checkbox"],
	  #wcfm-orders th input[type="checkbox"] {
	  ...
	  <div id="wcfm-orders-bulk-bar" style="display:none;margin-bottom:10px;padding:8px 12px;background:#f9f9f9;border:1px solid #e0e0e0;border-radius:4px;align-items:center;gap:10px;flex-wrap:wrap;">
	  	<span id="wcfm-orders-selected-count" style="font-weight:600;">0 <?php _e( 'selected', 'wc-frontend-manager' ); ?></span>
	  	<select id="wcfm-orders-bulk-status" style="padding:4px 8px;border-radius:3px;border:1px solid #ccc;">
	  		<option value=""><?php _e( '— Change Status —', 'wc-frontend-manager' ); ?></option>
	  		<?php foreach ( wc_get_order_statuses() as $status_key => $status_name ) : ?>
	  		<option value="<?php echo esc_attr( $status_key ); ?>"><?php echo esc_html( $status_name ); ?></option>
	  		<?php endforeach; ?>
	  	</select>
	  	<button id="wcfm-orders-bulk-apply" class="wcfm-btn btn-success" style="padding:4px 14px;"><?php _e( 'Apply', 'wc-frontend-manager' ); ?></button>
	  	<span id="wcfm-orders-bulk-result" style="margin-left:8px;"></span>
	  </div>
		...
		<div class="wcfm-container">
			<div id="wwcfm_orders_listing_expander" class="wcfm-content">
				<table id="wcfm-orders" class="display" cellspacing="0" width="100%">
					<thead>
						<tr>
							<th><input type="checkbox" id="wcfm-orders-check-all" style="vertical-align:middle;"></th>
