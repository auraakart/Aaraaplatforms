...
jQuery(document).ready(function($) {
	...
	if ( $( '#wcfm-customers-details-bookings' ).length > 0 ) {
	...
	if ( $( '#wcfm-customers-details-appointments' ).length > 0 ) {
	...
	if ( $( '#wcfm-customers-details-wallet' ).length > 0 ) {
		$wcfm_customers_wallet_table = $( '#wcfm-customers-details-wallet' ).DataTable( {
			"processing": true,
			"serverSide": true,
			"responsive": true,
			"pageLength": parseInt( dataTables_config.pageLength ),
			...
			"ajax": {
				...
				"data": function( d ) {
					d.action          = 'wcfm_ajax_controller';
					d.controller      = 'wcfm-customers-details-wallet';
					d.customer_id     = customer_id;
					d.wcfm_ajax_nonce = wcfm_params.wcfm_ajax_nonce;
				},
				"complete": function() {
					initiateTip();
					$( document.body ).trigger( 'updated_wcfm_customers_wallet' );
				}
			}
		} );
	}
	...
	if ( $( '#wcfm-notif-system' ).length > 0 ) {
		var $wcfmNotifTables = {};
		var wcfmNotifInited  = {};

		function wcfmInitNotifTable( tab ) {
			if ( wcfmNotifInited[ tab ] ) {
				if ( $wcfmNotifTables[ tab ] ) {
					$wcfmNotifTables[ tab ].columns.adjust().draw( false );
				}
				return;
			}
			wcfmNotifInited[ tab ] = true;

			$wcfmNotifTables[ tab ] = $( '#wcfm-customers-notif-' + tab ).DataTable( {
				"processing" : true,
				"serverSide" : true,
				"responsive" : true,
				"pageLength" : parseInt( dataTables_config.pageLength ),
				...
				"ajax": {
					...
					"data": function( d ) {
						d.action          = 'wcfm_ajax_controller';
						d.controller      = 'wcfm-customers-details-notifications';
						d.customer_id     = customer_id;
						d.notif_tab       = tab;
						d.wcfm_ajax_nonce = wcfm_params.wcfm_ajax_nonce;
					}
		...
		// Load the default (system) tab immediately
		wcfmInitNotifTable( 'system' );

		// Tab switching — lazy-init each tab on first click
		$( document ).on( 'click', '.wcfm-notif-tab', function() {
			var tab = $( this ).data( 'tab' );
			$( '.wcfm-notif-tab' ).removeClass( 'active' );
			$( this ).addClass( 'active' );
			$( '.wcfm-notif-pane' ).removeClass( 'active' );
			$( '#wcfm-notif-' + tab ).addClass( 'active' );
			wcfmInitNotifTable( tab );
		} );
	}
