...
class WCFMaf_Frontend {
	...
	public function __construct() {
		...
		// Date range support for affiliate list -> Admin dashboard
		add_filter('wcfmaf_affiliate_commission_query', array(&$this, 'apply_date_range_filter'), 10, 3);
	}
	function apply_date_range_filter($sql, $affiliate_id, $status) {
		global $wpdb;
		if (!empty($_POST['filter_date_form']) && !empty($_POST['filter_date_to'])) {
			...
			$time_filter = " AND DATE( created ) BETWEEN %s AND %s";
			$sql .= $time_filter;
  ...
  function wcfmaf_affiliate_product_commission_save( $new_product_id, $wcfm_products_manage_form_data ) {
	...
	function wcfmaf_affiliate_admin_order_commission( $order_id, $order_posted, $order ) {
		...
		if ( $order->get_meta( '_wcfmaf_order_processed' ) ) return;
		...
		$order->update_meta_data( '_wcfmaf_order_processed', 'yes' );
		$order->save();
	}
					...
					if( $vendor_affiliate_commission ) {
						...
						if( $cal_mode == 'on_commission' ) {
							...
							if( isset( $order_commission_rule['tax_enable'] ) && ( $order_commission_rule['tax_enable'] == 'yes' ) ) {
							    ...
							    $commission_tax = apply_filters('wcfmmp_commission_deducted_tax', $commission_tax, $vendor_id, $product_id, $variation_id, $order_id, $total_commission, $order_commission_rule, $order_item_id, 0);
