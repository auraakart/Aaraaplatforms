...
class WCFMa_Library {
  ...
  public function world_map_analytics_data() {
    ...
    if( !empty( $countries ) ) {
		...
		$wcfma_map_name_arr = $wcfminsights_viewname_arr = [];
		if(!is_array($wcfma_map_name)) {
			$wcfma_map_name_arr = array('wcfma_map_val'=>$wcfma_map_name);
		} 
		$wcfminsights_viewname_arr = array('viewname'=>__( 'views', 'wc-frontend-manager-analytics' ));
		wp_localize_script( 'wcfma_analytics_js', 'wcfminsights_countries', $country_arr_full );
		wp_localize_script( 'wcfma_analytics_js', 'wcfma_map_name', $wcfma_map_name_arr );
		wp_localize_script( 'wcfma_analytics_js', 'wcfminsights_viewname', $wcfminsights_viewname_arr );
  ...
  /**
	...
	*/
	public function load_jvectormap_lib() {
	  global $WCFM, $WCFMa;
	  wp_enqueue_script( 'jquery-jvectormap_js', $WCFMa->plugin_url . 'includes/jvectormap/jquery-jvectormap-2.0.5.min.js', array('jquery'), $WCFMa->version, true );
	  ...
	  wp_enqueue_style( 'wcfm_timepicker_css',  $WCFMa->plugin_url . 'includes/jvectormap/jquery-jvectormap-2.0.5.css', array(), $WCFMa->version );
	}
