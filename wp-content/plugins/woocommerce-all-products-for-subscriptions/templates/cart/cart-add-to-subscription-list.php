...
if ( empty( $subscriptions ) ) {
	...
	<div class="wcsatt-add-to-subscription-error" role="alert">
		<?php esc_html_e( 'No matching subscriptions found.', 'woocommerce-all-products-for-subscriptions' ); ?>
	</div>
	<?php
} else {
	?>
				...
				<?php foreach ( $subscriptions as $subscription_id => $subscription ) : ?>
					<tr class="order woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $subscription->get_status() ); ?>">
						...
						<td class="subscription-actions order-actions woocommerce-orders-table__cell woocommerce-orders-table__cell-subscription-actions woocommerce-orders-table__cell-order-actions">
							...
							<button type="submit" class="wcsatt-add-to-subscription-button button add alt" name="<?php echo 'cart' === $context ? 'add-cart-to-subscription' : 'add-to-subscription'; ?>" value="<?php echo absint( $subscription_id ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Add to subscription #%s', 'woocommerce-all-products-for-subscriptions' ), $subscription->get_order_number() ) ); ?>"><?php echo esc_attr( sprintf( __( 'Add to subscription #%s', 'woocommerce-all-products-for-subscriptions' ), $subscription->get_order_number() ) ); ?></button>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
