...
class WCFMd {
	...
	public $wcfmd_delivery_hub;
	public $deliveries_pdf;
	...
	function init() {
		...
		if( !defined('DOING_AJAX') ) {
			...
			$this->wcfmd_non_ajax = new WCFMd_Non_Ajax();
		}
	}

	public function declare_hpos_compatibility() {
		if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', 'wc-frontend-manager-delivery/wc-frontend-manager-delivery.php', true );
		}
	}
	...
	function init_wcfmd() {
		...
		if ( apply_filters( 'wcfm_is_pref_delivery', true ) ) {
			if ( !is_admin() || ( defined('DOING_AJAX') || defined('WCFM_REST_API_CALL') ) ) {
				$this->load_class( 'delivery-hub' );
				$this->wcfmd_delivery_hub = new WCFMd_Delivery_Hub();
			}
		}

		// Deliveries PDF run sheet — needs to hook its admin-ajax stream on every
		// request type, so it is loaded unconditionally.
		$this->load_class( 'deliveries-pdf' );
		$this->deliveries_pdf = new WCFMd_Deliveries_PDF();
	...
	function wcfmd_delivery_order_trash( $order_id ) {
		...
		if ( in_array( OrderUtil::get_order_type( $order_id ), wc_get_order_types(), true ) ) {
			$order = wc_get_order( $order_id );
	...
	function run_wcfmd_installer() {
		...
		// Hub table – runs independently so existing installs get it without re-triggering full installer
		if ( get_option( 'wcfmd_hub_table_install' ) !== '1.0.0' ) {
			$collate = $wpdb->has_cap( 'collation' ) ? $wpdb->get_charset_collate() : '';
			$wpdb->query(
				"CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}wcfm_delivery_hubs` (
			...
			update_option( 'wcfmd_hub_table_install', '1.0.0' );
		}
	}
