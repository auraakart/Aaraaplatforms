...
class WCFMd_Frontend {
	...
	public function __construct() {
		...
		if( apply_filters( 'wcfm_is_pref_delivery', true ) ) {
			...
			// One-time: insert Hub into wcfm_managed_menus just before Delivery Persons
			add_action( 'init', array( &$this, 'wcfmd_inject_hub_in_managed_menus' ), 100 );
			...
			if( apply_filters( 'wcfm_is_allow_delivery', true ) ) {
				...
				// Order Details Delivery View — always shown regardless of WCFM Ultimate
				add_action( 'end_wcfm_orders_details', array( &$this, 'end_wcfm_orders_details_load_views' ), 15 );

				// Order Delivery Boy Assigned — always registered so auto-assignment works in all contexts
  ...
  function wcfmd_delivery_wcfm_query_vars( $query_vars ) {
  	$wcfm_modified_endpoints = (array) get_option( 'wcfm_endpoints' );
		...
		$query_delivery_vars = array(
			...
			'wcfm-deliveries-pdf'         => ! empty( $wcfm_modified_endpoints['wcfm-deliveries-pdf'] ) ? $wcfm_modified_endpoints['wcfm-deliveries-pdf'] : 'deliveries-pdf',
  ...
  function wcfmd_delivery_endpoint_title( $title, $endpoint ) {
  	...
  	switch ( $endpoint ) {
			...
			case 'wcfm-deliveries-pdf' :
				$title = __( 'Deliveries PDF', 'wc-frontend-manager-delivery' );
			break;
  ...
  function wcfmd_delivery_init() {
  	global $WCFM_Query;
		...
		// Versioned so adding an endpoint re-flushes on installs that already
		// passed this gate — otherwise the new URL 404s until rules are flushed
		// by hand. Bump this whenever the endpoint list above changes.
		if( get_option( 'wcfm_updated_end_point_wcfma_delivery' ) != '2' ) {
			// Flush rules after endpoint update
			flush_rewrite_rules();
			update_option( 'wcfm_updated_end_point_wcfma_delivery', '2' );
		}
  }
  ...
  /**
	...
	function wcfmd_delivery_endpoints_slug( $endpoints ) {
		...
		$wcfma_delivery_endpoints = array(
													...
													'wcfm-deliveries-pdf'          => 'deliveries-pdf',
  ...
  function wcfmd_delivery_menus( $menus ) {
  	...
  	if( apply_filters( 'wcfm_is_allow_delivery', true ) ) {
			...
			$menus = array_slice($menus, 0, 3, true) +
												array( 'wcfm-delivery-hubs' => array( 'label'    => __( 'Delivery Hub', 'wc-frontend-manager-delivery'),
																														 'url'      => get_wcfmd_delivery_hubs_url(),
																														 'icon'     => 'map-marker-alt',
																														 'has_new'  => 'yes',
																														 'new_class'=> 'wcfm_sub_menu_items_delivery_hubs',
																														 'new_url'  => get_wcfmd_delivery_hubs_url(),
																														 'priority' => 52
																												) ) +
		...
		if( wcfm_is_delivery_boy() ) {
			$menus = array( 'wcfm-deliveries' => array(  'label'      => __( 'Deliveries', 'wc-frontend-manager-delivery'),
											...
											'wcfm-deliveries-pdf' => array( 'label'    => __( 'Deliveries PDF', 'wc-frontend-manager-delivery'),
																									 'url'        => get_wcfm_deliveries_pdf_url(),
																									 'icon'       => 'file-pdf',
																									 'priority'   => 54
																								 ) );
		} elseif( apply_filters( 'wcfm_is_allow_delivery', true ) ) {
			// Store owners and admins get the run sheet too, alongside Delivery Persons.
			$menus['wcfm-deliveries-pdf'] = array( 'label'    => __( 'Deliveries PDF', 'wc-frontend-manager-delivery'),
																						 'url'      => get_wcfm_deliveries_pdf_url(),
																						 'icon'     => 'file-pdf',
																						 'priority' => 54
																					);
  ...
  /**
   * One-time: insert Delivery Hub into wcfm_managed_menus just before Delivery Persons.
   * WCFM stores saved menu order in the DB; any item absent from that list gets a high
   * code-priority and sorts to the bottom. Running this once puts Hub in the right slot.
   */
  function wcfmd_inject_hub_in_managed_menus() {
    if ( get_option( 'wcfmd_hub_menu_injected' ) === '1.0' ) {
      return;
    }

    $managed = wcfm_get_option( 'wcfm_managed_menus', [] );

    // Nothing stored yet — code priorities will determine order on first page-load.
    if ( empty( $managed ) ) {
      update_option( 'wcfmd_hub_menu_injected', '1.0' );
      return;
    }

    // Already present — nothing to do.
    foreach ( $managed as $entry ) {
      if ( isset( $entry['slug'] ) && $entry['slug'] === 'wcfm-delivery-hubs' ) {
        update_option( 'wcfmd_hub_menu_injected', '1.0' );
        return;
      }
    }

    // Rebuild the array, inserting Hub just before Delivery Persons.
    $new_managed = [];
    $i           = 0;
    $inserted    = false;
    foreach ( $managed as $entry ) {
      if ( isset( $entry['slug'] ) && $entry['slug'] === 'wcfm-delivery-boys' ) {
        $new_managed[ $i ] = [
          'slug'     => 'wcfm-delivery-hubs',
    ...
    // Delivery Persons not in managed list — append Hub at end (better than nothing).
    if ( ! $inserted ) {
      $new_managed[ $i ] = [
        'slug'     => 'wcfm-delivery-hubs',
        'label'    => __( 'Delivery Hub', 'wc-frontend-manager-delivery' ),
    ...
    wcfm_update_option( 'wcfm_managed_menus', $new_managed );
    update_option( 'wcfmd_hub_menu_injected', '1.0' );
  }
  ...
  public function wcfmd_delivery_boy_assigned( $order_id, $order_item_id, $wcfm_tracking_data, $product_id ) {
  	...
  	if( $wcfm_delivery_boy ) {
  		...
  		if( wcfm_is_vendor() ) {
  			...
  			if( apply_filters( 'wcfm_is_allow_itemwise_notification', true ) ) {
					...
					if( ( $vendor_id && !$order->get_meta( '_wcfm_order_delivery_assigned_'.$vendor_id, true ) ) || ( !$vendor_id && !$order->get_meta( '_wcfm_order_delivery_assigned_'.$order_id, true ) ) ) {
      ...
      if( $vendor_id ) {
		$order->update_meta_data( '_wcfm_order_delivery_assigned_'.$vendor_id, 'yes' );
		$order->update_meta_data( '_wcfm_order_delivery_assigned_'.$order_id, 'yes' );
      } else {
		$order->update_meta_data( '_wcfm_order_delivery_assigned_'.$order_id, 'yes' );
      }
	  $order->save();
