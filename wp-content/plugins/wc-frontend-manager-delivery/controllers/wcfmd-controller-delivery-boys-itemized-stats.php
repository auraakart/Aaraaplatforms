...
class WCFMd_Delivery_Boys_Stats_Controller {
	...
	public function processing() {
		...
		$count_args = array( $delivery_boy_id );
		if ( $status ) {
			$sql .= " AND delivery_status = %s";
			$count_args[] = $status;
		}
		$delivery_count = $wpdb->get_var($wpdb->prepare($sql, $count_args));

		// Get Product Count
		$sql  = "SELECT * FROM `{$wpdb->prefix}wcfm_delivery_orders`";
		$sql .= " WHERE 1=1";
		$sql .= " AND delivery_boy = %d";
		$query_args = array( $delivery_boy_id );
		if ( $status ) {
			$sql .= " AND delivery_status = %s";
			$query_args[] = $status;
		}
		...
		$query_args[] = $length;
		$query_args[] = $offset;
		$wcfm_delivery_orders_array = $wpdb->get_results($wpdb->prepare($sql, $query_args));
		if (defined('WCFM_REST_API_CALL')) {
