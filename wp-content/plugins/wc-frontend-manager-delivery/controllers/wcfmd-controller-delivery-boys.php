...
class WCFMd_Delivery_Boys_Controller {
	...
	public function processing() {
		...
		if (!empty($_POST['delivery_boy_hub'])) {
			$hub_meta = array(
				'key'     => '_wcfmd_delivery_hub',
				'value'   => absint($_POST['delivery_boy_hub']),
				'compare' => '='
			);
			if (isset($args['meta_query'])) {
				$args['meta_query'][] = $hub_meta;
			} else {
				$args['meta_query'] = array($hub_meta);
			}
		}
		...
		if (!empty($wcfm_delivery_boys_array)) {
			foreach ($wcfm_delivery_boys_array as $wcfm_delivery_boys_single) {
				...
				// Hub
				$boy_hub_id = (int) get_user_meta($wcfm_delivery_boys_single->ID, '_wcfmd_delivery_hub', true);
				if ($boy_hub_id) {
					$hub_row = $wpdb->get_row($wpdb->prepare("SELECT name FROM {$wpdb->prefix}wcfm_delivery_hubs WHERE ID = %d", $boy_hub_id));
					$wcfm_delivery_boys_json_arr[$index][] = $hub_row ? '<span class="wcfm_delivery_hub">' . esc_html($hub_row->name) . '</span>' : '&ndash;';
				} else {
					$wcfm_delivery_boys_json_arr[$index][] = '&ndash;';
				}
