		...
		if (isset($wcfm_delivery_boy_manager_form_data['user_name']) && !empty($wcfm_delivery_boy_manager_form_data['user_name'])) {
			if (isset($wcfm_delivery_boy_manager_form_data['user_email']) && !empty($wcfm_delivery_boy_manager_form_data['user_email'])) {
				...
				if (!$has_error) {
					...
					$user_data = array(
						'user_login'     => $wcfm_delivery_boy_manager_form_data['user_name'],
					...
					if (!$delivery_boy_id) {
						...
						// Delivery Hub
						if ( !empty( $wcfm_delivery_boy_manager_form_data['wcfmd_delivery_hub'] ) ) {
							update_user_meta( $delivery_boy_id, '_wcfmd_delivery_hub', absint( $wcfm_delivery_boy_manager_form_data['wcfmd_delivery_hub'] ) );
						} else {
							delete_user_meta( $delivery_boy_id, '_wcfmd_delivery_hub' );
						}
					...
					if (!$has_error) {
						echo '{"status": true, "message": "' . $wcfm_delivery_boy_messages['delivery_boy_saved'] . '", "redirect": "' . apply_filters('wcfm_delivery_boy_manage_redirect', get_wcfm_delivery_boys_manage_url($delivery_boy_id), $delivery_boy_id) . '"}';
					} else {
						echo '{"status": false, "message": "' . $wcfm_delivery_boy_messages['delivery_boy_failed'] . '"}';
					}
				}
			} else {
