...
class WCFMd_Delivery_Boys_Stats_Controller {
	...
	public function processing() {
		...
		$delivery_boy_id = !empty($_POST['wcfm_delivery_boy']) ? absint($_POST['wcfm_delivery_boy']) : '';
		$status          = !empty($_POST['status_type']) ? sanitize_text_field($_POST['status_type']) : '';

		$sql  = "SELECT COUNT(ID) FROM `{$wpdb->prefix}wcfm_delivery_orders`";
		$sql .= " WHERE 1=1";
		$sql .= " AND delivery_boy = %d";
		$sql .= " AND is_trashed = 0";
		if ($status) $sql .= " AND delivery_status = %s";
		$sql .= " GROUP BY order_id, vendor_id";
		$total_item_results = $wpdb->get_results($wpdb->prepare($sql, $delivery_boy_id, $status));
		$delivery_count = 0;
		...
		$wcfm_delivery_orders_array = $wpdb->get_results($wpdb->prepare($sql, $delivery_boy_id, $status, $length, $offset));
		if (defined('WCFM_REST_API_CALL')) {
		...
		if (!empty($wcfm_delivery_orders_array)) {
			foreach ($wcfm_delivery_orders_array as $wcfm_delivery_order_single) {
				...
				if (apply_filters('wcfm_is_pref_delivery_time', true) && apply_filters('wcfm_is_allow_delivery_time', true)) {
					$wcfmd_delvery_times = $the_order->get_meta('_wcfmd_delvery_times', true);
