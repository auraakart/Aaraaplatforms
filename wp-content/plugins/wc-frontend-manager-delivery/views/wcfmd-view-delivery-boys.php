  ...
  <div class="wcfm-page-headig">
	...
	<div class="wcfm-collapse-content">
	  ...
	  <div class="wcfm_delivery_boys_filter_wrap wcfm_filters_wrap">
			...
			// Hub filter
			$_filter_hubs = $wpdb->get_results( "SELECT ID, name FROM {$wpdb->prefix}wcfm_delivery_hubs WHERE status='active' ORDER BY name ASC" );
			if ( $_filter_hubs ) {
				$_hub_filter_opts = array( '' => __( 'All Hubs', 'wc-frontend-manager-delivery' ) );
				foreach ( $_filter_hubs as $_fh ) { $_hub_filter_opts[ $_fh->ID ] = esc_html( $_fh->name ); }
				$WCFM->wcfm_fields->wcfm_generate_form_field( array(
					"dropdown_delivery_hub" => array( 'type' => 'select', 'options' => $_hub_filter_opts, 'attributes' => array( 'style' => 'width: 180px; margin-left: 8px;' ) )
				) );
			}
			?>
		</div>
			...
			<div id="wcfm_delivery_boys_expander" class="wcfm-content">
				<table id="wcfm-delivery-boys" class="display" cellspacing="0" width="100%">
					<thead>
						...
						<th><?php _e( 'Hub', 'wc-frontend-manager-delivery' ); ?></th>
