...
if( isset( $wp->query_vars['wcfm-delivery-boys-manage'] ) && !empty( $wp->query_vars['wcfm-delivery-boys-manage'] ) ) {
	...
	if($staff_user && !empty($staff_user)) {
		...
		$delivery_hub_id = (int) get_user_meta( $delivery_boy_id, '_wcfmd_delivery_hub', true );
...
// Build hub options for the dropdown
$hub_options = array( '' => __( '— Select Hub —', 'wc-frontend-manager-delivery' ) );
$_hubs = $wpdb->get_results( "SELECT ID, name FROM {$wpdb->prefix}wcfm_delivery_hubs WHERE status='active' ORDER BY name ASC" );
foreach ( $_hubs as $_h ) { $hub_options[ $_h->ID ] = esc_html( $_h->name ); }
						...
						<?php
						  if( $delivery_boy_id ) {
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_delivery_manager_fields_general', array(  
																																						...
																																						"wcfmd_delivery_hub" => array( 'label' => __('Delivery Hub', 'wc-frontend-manager-delivery'), 'type' => 'select', 'options' => $hub_options, 'class' => 'wcfm-select wcfm_ele', 'label_class' => 'wcfm_ele wcfm_title', 'value' => $delivery_hub_id ),
