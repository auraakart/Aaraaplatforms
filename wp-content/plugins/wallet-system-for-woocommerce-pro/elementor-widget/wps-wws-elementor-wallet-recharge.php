...
if ( wc_post_content_has_shortcode( 'WPS_WALLET_RECHARGE' ) ) {
	...
	if ( ! empty( $product_id ) && ! empty( $enable_wallet_recharge ) ) {
		echo '<div class="content wps_wallet_shortcodes">
			<h3>' . esc_html__( 'Wallet Recharge', 'wallet-system-for-woocommerce-pro' ) . '</h3>
			<form method="post" action="" id="wps_wallet_transfer_form">
				<p class="wps-wallet-field-container form-row form-row-wide">
					<label for="wps_wallet_recharge_amount">' . esc_html__( 'Enter Amount (', 'wallet-system-for-woocommerce-pro' );
		echo esc_html( get_woocommerce_currency_symbol( $current_currency ) ); echo ')</label>
					<input type="number" id="wps_wallet_recharge" step="0.01" data-min="' . esc_attr( $min_value ) . '" data-max="' . esc_attr( $max_value ) . '" name="wps_wallet_recharge_amount" required="">
				</p>
				<p class="error"></p>
				<p class="wps-wallet-field-container form-row">
					...
					<input type="submit" class="wps-btn__filled button" id="wps_recharge_wallet" name="wps_recharge_wallet" value="' . esc_html__( 'Proceed', 'wallet-system-for-woocommerce-pro' ) . '">
				</p>
			</form>
		</div>';
	}
