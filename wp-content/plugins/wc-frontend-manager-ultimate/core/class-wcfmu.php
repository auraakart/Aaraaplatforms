...
class WCFMu {
    ...
    public $wcfmu_subscription_create;

    public $wcfmu_customer_addresses;
    ...
    function init_wcfmu() {
        ...
        if (wcfm_is_subscription()) {
            ...
            // Subscription Create — REST API + custom schedule hooks (always loaded)
            $this->load_class('subscription-create');
            $this->wcfmu_subscription_create = new WCFMu_Subscription_Create();
        }

        // Load Wallet Module
        if (!is_admin() || defined('DOING_AJAX')) {
            $this->load_class('wallet');
            $this->wcfmu_wallet = new WCFMu_Wallet();
        }
        ...
        // Customer multiple delivery addresses
        $this->load_class('customer-addresses');
        $this->wcfmu_customer_addresses = new WCFMu_Customer_Addresses();

        // Mobile OTP login & register (REST API + shortcodes)
        $this->load_class('mobile-auth');
        $this->wcfmu_mobile_auth = new WCFMu_Mobile_Auth();
