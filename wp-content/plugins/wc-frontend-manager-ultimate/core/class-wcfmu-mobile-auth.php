...
class WCFMu_Mobile_Auth {
	...
	private function send_via_custom_sms( $phone, $otp, $vendor_id = 0 ) {
		...
		// Mirror the attempt into the Aaraa SMS log (AaraaNotifications → SMS) so
		// app-login OTPs are visible alongside order/wallet messages.
		if ( class_exists( '\\Aaraa\\Admin\\Notifications_Admin' ) ) {
			$http_code = is_wp_error( $resp ) ? '' : (string) wp_remote_retrieve_response_code( $resp );
			$body_resp = is_wp_error( $resp )
				? $resp->get_error_message()
				: trim( wp_strip_all_tags( (string) wp_remote_retrieve_body( $resp ) ) );
			$ok = ! is_wp_error( $resp ) && (int) wp_remote_retrieve_response_code( $resp ) < 400;

			$uid = \Aaraa\Admin\Notifications_Admin::user_id_for_phone( $phone );

			\Aaraa\Admin\Notifications_Admin::log( array(
				'type'      => 'otp',
				'ref'       => $uid ? (string) $uid : '',
				'mobile'    => $phone,
				'message'   => $body ? $body : 'OTP ' . $otp,
