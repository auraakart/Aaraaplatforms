...
class WCFMu_Ajax
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_wcfm_bulk_subscription_status', [ &$this, 'wcfm_bulk_subscription_status' ]);
    ...
    public function wcfm_bulk_subscription_status() {
        if (!check_ajax_referer('wcfm_ajax_nonce', 'wcfm_ajax_nonce', false)) {
            wp_send_json_error(__('Invalid nonce! Refresh your page and try again.', 'wc-frontend-manager-ultimate'));
            wp_die();
        }
        ...
        $sub_ids    = isset($_POST['sub_ids']) ? array_filter(array_map('absint', (array) $_POST['sub_ids'])) : [];
        $new_status = isset($_POST['sub_status']) ? sanitize_key($_POST['sub_status']) : '';

        $valid_statuses = ['active', 'on-hold', 'cancelled', 'expired', 'pending-cancel', 'pending'];
        if (empty($sub_ids)) {
            wp_send_json_error(__('No subscriptions selected.', 'wc-frontend-manager-ultimate'));
            wp_die();
        }
        if (!in_array($new_status, $valid_statuses, true)) {
            wp_send_json_error(__('Invalid subscription status.', 'wc-frontend-manager-ultimate'));
            wp_die();
        }
        ...
        foreach ($sub_ids as $sub_id) {
            ...
            try {
                ...
                // Skip subscriptions that cannot transition to the requested status
            }
        }
        ...
        $status_labels = [
            ...
            'pending-cancel' => __('Pending Cancellation', 'wc-frontend-manager-ultimate'),
        ...
        wp_send_json_success(['message' => sprintf(
            _n('%d subscription updated to %s.', '%d subscriptions updated to %s.', $updated, 'wc-frontend-manager-ultimate'),
            $updated,
            $status_label
        )]);
        wp_die();
