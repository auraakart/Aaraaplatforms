    ...
    public function processing() {
        ...
        if (isset($wcfm_orders_manage_form_data['associate_products']) && !empty($wcfm_orders_manage_form_data['associate_products'])) {
            ...
            try {
                ...
                $oid = base64_encode($order_id);
                $payment_link = 'https://thefarmersvillage.com/make_payment.php?oid='.$oid;
                
                $billing_email  = $order->get_billing_email();
                $billing_phone  = $order->get_billing_phone();
                $billing_first_name = $order->get_billing_first_name();
                $billing_last_name  = $order->get_billing_last_name();
                $person_name = $billing_first_name.' '.$billing_last_name;
                ...
                $customer_contact_number = $billing_phone;
                //$total_order_amount = $order->get_total();
                
                $refund_amount = 0;
                $total_amount = $order->get_total();
                $refund_amount = $order->get_total_refunded();
                $total_order_amount =  $total_amount-$refund_amount;
                 
                
                
                $mbl_count = strlen($customer_contact_number);
                
                
                if($mbl_count == 10){
                $invoice = wcpdf_get_document( 'invoice', $order, true );
                $invoice_file_name = time().'-'.$order_id.'.pdf';
                $pdf_path = get_home_path().'order-invoices/'.$invoice_file_name;
                $pdf_data = $invoice->get_pdf(); 
                file_put_contents ( $pdf_path, $pdf_data );		
                $invoice_link = 'https://thefarmersvillage.com/order-invoices/'.$invoice_file_name;
                
$whatsapp_msg = '';
$whatsapp_msg .= '
Hi '.$billing_first_name.' '.$billing_last_name.', 

Your subscription/one-time purchase order has been processed/renewed (Order ID - '.$order_id.'). Your payment for the order Rs.'.$total_order_amount.' is pending. Kindly requesting to make the payment immediately. 

Payment Link:  '.$payment_link.'

Invoice Link:  '.$invoice_link.'

Login Link to pay: https://thefarmersvillage.com/my-account/ 

Android play store link to our App : https://rb.gy/3gh40v

The Farmers Village Customer Care 
(+91) 93448 81208 
support@thefarmersvillage.com 
https://thefarmersvillage.com';
                
                //UPI number for payments: 8667064149
                //echo $whatsapp_msg;	
                
                
                /* whatsapp code  */
                $to = '91'.$customer_contact_number;
                $referenceId = '657f07bd68342487ed6fca4e';
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'https://whatsapp.myappstores.com/api/sendText?token='.$referenceId.'&phone='.$to.'&message='.urlencode($whatsapp_msg));
                ...
                curl_close($ch);
                /* whatsapp code ends */
                $order_note = 'Whastapp message sent on '.date('d-M-Y h:i a');
                $order->add_order_note($order_note);
                
               /*
                global $wpdb;     
                $table_name = $wpdb->prefix . 'whatsapp_reminder';     
                $wpdb->insert($table_name, array('order_id'=>$order_id,'first_name'=>$billing_first_name,'last_name'=>$billing_last_name,'email_id'=>$billing_email,'contact_number'=>$billing_phone,'total_amount'=>$total_order_amount,
                'created_at'=>time(),'whatsapp_request'=>$whatsapp_msg,'whatsapp_response'=>json_encode($result)));
                */
                }
                /* whatsapp message ends here */
