    ...
    public function processing()
    {
        ...
        if (! empty($wcfm_subscriptions_array)) {
            ...
            foreach ($wcfm_subscriptions_array as $wcfm_subscriptions_single) {
                ...
                // Status
                $wcfm_subscriptions_json_arr[$index][] = apply_filters('wcfm_subscription_status_label', '<span class="subscription-status tips wcicon-status-'.sanitize_title($the_subscription->get_status()).' text_tip" data-tip="'.$wc_get_subscription_status_name[$the_subscription->get_status()].'" data-sub-id="'.esc_attr($wcfm_subscriptions_single->ID).'"></span>', $wcfm_subscriptions_single->ID, $the_subscription);
