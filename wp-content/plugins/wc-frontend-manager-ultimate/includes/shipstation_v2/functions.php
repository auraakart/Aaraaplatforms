 ...
 * @return array
 */
function wcfmu_shipstation_v2_get_orders($vendor_id, $args = []) {
    global $wpdb;

    $current_time = current_time('mysql');
    $page         = isset($args['page']) ? max(1, absint($args['page'])) : 1;
    $per_page     = isset($args['per_page']) ? max(1, absint($args['per_page'])) : WCFMu_SHIPSTATION_EXPORT_LIMIT;
    ...
    if (OrderUtil::custom_orders_table_usage_is_enabled()) {
        ...
        $map['modified'] = 'post_modified_gmt';
    }
    ...
    $date_column = ('modified' === $args['date_type']) ? $map['modified'] : $map['date'];
    $order_column = ('modified' === $args['order_by']) ? $map['modified'] : $map['date'];
    $sort_order  = ('DESC' === strtoupper($args['order'])) ? 'DESC' : 'ASC';
    ...
    if (false === $orders) {
        ...
        if (is_array($args['status'])) {
            ...
            $where .= sprintf(" AND order_status IN ('%s')", implode("', '", $statuses));
        } else if ($args['status']) {
            $where .= $wpdb->prepare(' AND order_status = %s', $args['status']);
        }

        $where .= $wpdb->prepare(" AND p.{$date_column} >= %s AND p.{$date_column} <= %s", $args['start_date'], $args['end_date']);
        ...
        if (!$args['count']) {
            ...
            $order_by = " ORDER BY p.{$order_column} {$sort_order}";
        ...
        wcfmu_v2_cache_update_group($cache_key, $cache_group);
    } //end if

    return $orders;
} //end wcfmu_shipstation_v2_get_orders()
 ...
 * @return void
 */
function wcfmu_v2_cache_update_group($key, $group) {
...
} //end wcfmu_v2_cache_update_group()
 ...
 * @return array
 */
function wcfmu_v2_get_order_commission_ids_by_vendor($vendor_id, $order_id) {
    global $wpdb;
...
} //end wcfmu_v2_get_order_commission_ids_by_vendor()
 ...
 */
function shipstation_v2_update_order_status($vendor_id, $order_id, $order_status) {
    global $WCFM;
...
} //end shipstation_v2_update_order_status()
 ...
 * @return string json_format
 */
function shipstation_v2_vendor_order_status_update($vendor_id, $order_id, $order_status) {
    ...
    if ($order_sync == 'yes') {
        return shipstation_v2_update_order_status($vendor_id, $order_id, $order_status);
    }
...
} //end shipstation_v2_vendor_order_status_update()


/**
 * Mark WCfM Marketplace order as Shipped
 */
function shipstation_v2_order_mark_shipped($order_id, $vendor_id, $shipstation_data, $terminate = true) {
    ...
    if (!empty($order_id)) {
        ...
        $line_items = shipstation_v2_valid_line_items($order->get_items('line_item'), $order_id, $vendor_id);
            ...
            if (wcfm_is_vendor($vendor_id)) {
                ...
                if ($order_sync != 'yes') {
                    foreach ($order_item_ids as $order_item_id) {
                        $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wcfm_marketplace_orders SET commission_status = 'shipped', shipping_status = 'shipped' WHERE order_id = %d AND vendor_id = %d AND item_id = %d", $order_id, $vendor_id, $order_item_id));
                    }
                }
...
} //end shipstation_v2_order_mark_shipped()


// Filter Order Details Line Items as Per Vendor
function shipstation_v2_valid_line_items($items, $order_id, $vendor_id) {
    global $WCFM, $wpdb;

    $sql            = $wpdb->prepare("SELECT `product_id`, `item_id` FROM {$wpdb->prefix}wcfm_marketplace_orders WHERE `vendor_id` = %d AND `order_id` = %d", $vendor_id, $order_id);
    ...
    return $valid;
}//end shipstation_v2_valid_line_items()

/**
 * Ensure the marketplace object is initialized with the current vendor context.
 *
 * @param int $vendor_id Vendor user ID.
 *
 * @return void
 */
function wcfmu_shipstation_v2_bootstrap_marketplace($vendor_id) {
    global $WCFM;

    $vendor_id = absint($vendor_id);

    if (!$vendor_id || empty($WCFM) || !is_object($WCFM) || !function_exists('wcfm_is_vendor') || !wcfm_is_vendor($vendor_id)) {
        return;
    }

    if (!$WCFM->wcfm_marketplace && !empty($WCFM->is_marketplace)) {
        $WCFM->load_class($WCFM->is_marketplace);

        if (class_exists('WCFM_Marketplace')) {
            $WCFM->wcfm_marketplace = new WCFM_Marketplace();
        }
    }

    if ($WCFM->wcfm_marketplace && property_exists($WCFM->wcfm_marketplace, 'vendor_id')) {
