	...
	Plugin.prototype = {
		...
		auth: function (callback) {
			...
			if ( this.data.ref == null ) {
				try {
					var db_url = this.opts.database_url ? this.opts.database_url.replace( /\/$/, '' ) : 'https://' + this.opts.app_id + '.firebaseio.com';
					this.data.ref       = new Firebase( db_url );
					this.data.ref_conn  = new Firebase( db_url + '/.info/connected' );
					this.data.ref_cnv   = new Firebase( db_url + '/chat_sessions' );
					this.data.ref_msgs  = new Firebase( db_url + '/chat_messages' );
			...
			this.data.auth      = this.data.ref.authWithCustomToken(
				...
				function (error) {
					if (error) {
						...
						if (error.code === 'EXPIRED_TOKEN' || error.code === 'INVALID_TOKEN' || error.code === 'auth/id-token-expired' || error.message.indexOf('expired') !== -1) {
							$( '#FBC_notify' ).html( self.strings.msg.connecting + '... (Refreshing Token)' ).show();
							self.post(
								'fbc_ajax_callback',
								'get_token',
								{},
								function (r) {
									if ( ! r.error && r.token ) {
										...
										$( '#FBC_notify' ).hide().html( 'Session expired. Please reload the page.' ).fadeIn( 200 );
									}
								}
							);
							return;
		...
		post: function (action, mode, data, callback) {
			...
			$.post( fbc.ajax_url + '?action=' + action + '&mode=' + mode, post_data, callback, 'json' )
				.fail(
					function (jqXHR) {
						// Log error
