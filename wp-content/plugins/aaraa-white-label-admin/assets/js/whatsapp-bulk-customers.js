/**
 * Bulk "Send WhatsApp" for the Aaraa Customer 360 list.
 *
 * Adds a button to the list toolbar. Clicking it opens a modal where you pick an
 * approved template, map its variables/buttons to customer tokens, preview the
 * message, and send it to the selected customers. Tokens are resolved per
 * customer on the server; each send is logged, and a customer already sent this
 * exact template today is skipped.
 *
 * Shares the AaraaWABulk config shape with the Orders sender, so the modal code
 * is intentionally the same — only the selection source and the id field differ.
 */
( function ( $ ) {
	'use strict';
	...
	function addButton() {
		var $bar = $( '.tablenav.top' ).first();
		if ( ! $bar.length || $bar.find( '.aaraa-wa-bulk-btn' ).length ) { return; }
		var $wrap = $( '<div class="alignleft actions aaraa-wa-cust-bar"></div>' );
	...
	function injectCss() {
		var css =
			'.aaraa-wa-cust-bar{margin-right:6px;}' +
			'.aaraa-wa-bulk-btn{margin:0;}' +
		...
		$( '<style id="aaraa-wa-cust-css"></style>' ).text( css ).appendTo( 'head' );
	}
